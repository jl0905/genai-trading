...
// Approximate monospace character dimensions (in pixels) for grid sizing.
...
export default function EntryVisual() {
	...
	useEffect(() => {
		...
		const updateDims = () => {
			...
			const cols = Math.max(100, Math.floor(rect.width / CHAR_WIDTH_PX));
			const rows = Math.max(50, Math.floor(rect.height / CHAR_HEIGHT_PX));
			setGridDims({ width: cols, height: rows });
		};
