...
- **SplineTab.jsx** *(Multi tab)*: Re-implements the home tab's 3D rendering concepts using custom hardware-accelerated geometries via native **Three.js**. Features a rotating staircase formation of 10 cubes (4→3→2→1 layout) utilizing ultra-smooth `RoundedBoxGeometry` (`radius: 0.08`, `segments: 12`), rendered using premium pristine `MeshPhysicalMaterial` exhibiting high-end iridescent solid glass properties (`transmission: 0.85`, `opacity: 1.0`, `roughness: 0.15`, `clearcoat: 1.0`, `clearcoatRoughness: 0.0`, independent material cloning per cube supporting an animated diagonal color wave sweeping smoothly between custom website theme red `#550000` and rich vibrant green `#008833` shades live using normalized position-dependent offsets via `lerpColors`, `ior: 1.5`, `thickness: 1.5`, `dispersion: 2.5`, full thin-film holographic iridescence, and volumetric absorption via `attenuationColor` / `attenuationDistance`) dynamically set against a theme-responsive contrasting background (`#f0f4f8` in light mode, `#0f172a` in dark mode via a live `MutationObserver`) and illuminated via a complete `RoomEnvironment` enriched with sharp directional bevel highlights, brilliant camera-facing rim lighting, and vibrant pink/cyan `RectAreaLight` gradients. The entire staircase rotates as a single rigid body supporting click-and-drag manual rotation alongside intuitive trackpad pinch-to-zoom and mouse wheel camera scaling controls. **Additionally integrates a novel recursive tileable interface overlay** that sits on top of the background visuals without obstructing mouse drag interactions. Displays a theme-aware dotted selection border (black in light mode, white in dark mode) enveloping the currently selected area, which can be dynamically subdivided recursively by pressing **W** (top half), **S** (bottom half), **A** (left half), **D** (right half), undone with **Z**, and reset to the full content area via **R** or **Escape**. Clicking the `+ Place Component Here` overlay within any active sub-tile opens a placement menu allowing users to embed a compact instance of the Interactive Chart (`TvInteractiveChart` with `isCompact` mode) directly into the subdivided tile region. Compact chart instances automatically hide AI Analysis, Educate mode, Visible Window, and Last Updated sections, and collapse indicator toggles into a dropdown menu to fit the constrained space.
